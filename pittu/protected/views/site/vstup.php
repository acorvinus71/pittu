...
<?php $this->pageTitle=Yii::app()->name . 'Намунаи маводҳои дохилшавӣ';
$this->breadcrumbs=array(
	Yii::t('translate','Намунаи маводҳои дохилшавӣ'),
);
?>
<?php
require_once dirname(__FILE__) . '/../site/conn.php';
$sql=mysqli_query($link,"select * from vstup");
...
$sql=mysqli_query($link,"select * from namuna");
while($res=mysqli_fetch_array($sql)){
	if($l=='en')
	echo "<a href=".$res[4].'><p class="text_teach">'.$res[3].'</p></a>';
	elseif($l=='ru')
	echo "<a href=".$res[4].'><p class="text_teach">'.$res[2].'</p></a>';
	else
	echo "<a href=".$res[4].'><p class="text_teach">'.$res[1].'</p></a>';
	}
?>
